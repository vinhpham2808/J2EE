...
const NotificationDropdown = () => {
  ...
  const pollNotifications = useCallback(async () => {
    try {
      ...
      console.error("Real-time notification sync error", error);
    }
  }, [triggerToast]);
      ...
      {showDropdown && (
        <div className="fixed top-16 right-4 left-4 sm:absolute sm:top-auto sm:right-0 sm:left-auto sm:mt-2 sm:w-80 rounded-2xl shadow-2xl z-50 overflow-hidden
          ...
          <div className="px-4 py-3 border-b border-slate-100 dark:border-white/10 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
            <h3 className="font-semibold text-slate-800 dark:text-white">{t("notifications.title")}</h3>
            {unreadCount > 0 && (
              <button
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="py-10 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">{t("notifications.empty")}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{t("notifications.allUpToDate")}</p>
              </div>
            )}
          </div>
          ...
          <div className="p-2 border-t border-slate-100 dark:border-white/10 bg-white dark:bg-[#1E293B]">
            ...
            >
              {t("notifications.viewAll")} <ArrowRight size={14} />
            </button>
          </div>
